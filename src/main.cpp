      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
			...
			for (int i=0; i< sensor_fusion.size(); i++)
			{
				...
				if (d < (2+4*lane +2) && d > (2+4*lane-2))
				{
					...
					{	// if we are too close we need to break
						...
						cout << "TOO CLOSE!!!!!\n";
						ref_vel -= .45; // add some negative accelration for emergency break
						break;
					}
			...
			if ((too_close) || (ref_vel > target_vel*0.98))
			{
				ref_vel -= .15;
			}
			else if (ref_vel < target_vel*0.98)
			{
				if (ref_vel < 10){ref_vel += 0.3;} // extra acceleration for begining of drive
				ref_vel += .6;
			}

          	json msgJson;
						
			// Create a list of widely spaced (x,y) waypoints evenly spaced at 30m. later we will interpoolate them
			vector<double> ptsx;
			vector<double> ptsy;

			// reference x, y, yaw status. either we will reference the starting point as where the car is or at the previous paths end point
			double ref_x = car_x;
			double ref_y = car_y;
			...
			// Start will all the previous path points
			...
			// fill up the rest of our path planner after filling it with previous points up to 50
